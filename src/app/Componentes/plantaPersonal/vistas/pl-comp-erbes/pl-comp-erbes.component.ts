...
@Component({
  selector: 'app-pl-comp-erbes',
  templateUrl: './pl-comp-erbes.component.html',
  styleUrls: ['./pl-comp-erbes.component.css']
})
export class PlCompErbesComponent implements OnInit {
  dataPlanta?: any[];
  permanente?: any[];
  total1Per: any;
  total2Per: any;
  ...
  filterData1(formData) {
    ...
    const datosFiltrados3 = this.dataPlanta.filter(
      (dato) =>
        new Date(dato.fecha).getMonth() ===
        new Date(formData.startDate1).getMonth() - 1
    );
    const dfiltro3 = datosFiltrados3.filter(
      (dato) =>
        new Date(dato.fecha).getFullYear() ===
        new Date(formData.startDate1).getFullYear()
    );
    ...
    const dfiltrado3 = dfiltro3.map((obj) => ({
      ...obj,
      cantidad1: 0,
      ...obj,
      cantidad2: 0,
    ...
    const datosFiltrados = dFiltrado1.concat(dfiltrado2, dfiltrado3);
    ...
    this.permanente = plantaPer.reduce((acc, item) => {
      ...
      if (existingItem) {
        ...
        existingItem.cantidad3 += item.cantidad3;
      } else {
        acc.push({
    ...
    this.total3Per = this.permanente.reduce(
      (acc, item) => acc + item.cantidad3,
      0
    );
    ...
    this.total3Tem = this.temporario.reduce(
      (acc, item) => acc + item.cantidad3,
      0
    );
    ...
    this.total3Sup = this.suplente.reduce(
      (acc, item) => acc + item.cantidad3,
      0
    );
  }
